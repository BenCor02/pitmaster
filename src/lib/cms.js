...
function normalizeSettingsRow(row) {
  if (!row) return {}
  // PATCH: compatibilité entre ancien modèle key/value et nouveau modèle en ligne unique
...
export async function upsertSiteSettingsRow(payload) {
  ...
  const legacyRows = Object.entries(payload).map(([key, value]) => ({ key, value: String(value ?? '') }))
  const legacy = await supabase.from('site_settings').upsert(legacyRows, { onConflict: 'key' }).select()
